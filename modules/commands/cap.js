...
module.exports ={
	...
	run: async function({ api, event, args, Users }) {
		try {
			...
			const Arraykey = [
				"10c1369fe89f4bb4a0aa201fe507d188",
				"b6974d25e42e4a5aa92e2ca8ce66eb47"
			]
			var key = Arraykey[Math.floor(Math.random() * Arraykey.length)];////Lên web https://api.apiflash.com để lấy key nhé
			if (args[0] == 'web') {
				send(`[🙀]➜ ${await Users.getNameUser(senderID)}, vui lòng đợi cap web có thể mất đến 30s`, threadID, messageID)
				var body = [], attachment = [], i = 1, file = []
				body += '==== [ CAP WEB ] ====\n[🙀]➜ Đã cap thành công cho web:\n';
