...
module.exports.onLoad = () => {
  ...
  if (!fs.existsSync(__dirname + "/data/shortimage.json")) fs.writeFileSync(__dirname + "/data/shortimage.json", JSON.stringify([]), 'utf-8');
}
...
module.exports.handleEvent = async function({ api, event }) {
  ...
  if (event.type !== "message_unsend" && event.body.length !== -1) {
    let shortcut = JSON.parse(fs.readFileSync(__dirname + "/data/shortimage.json"));
      ...
      if (getThread.some(item => item.in == event.body)) {
        ...
        if (typeof shortOut == "string") {
          ...
          if (shortOut.indexOf(" | ") !== -1) {
            ...
            fs.writeFileSync( __dirname + `/data/shortimage${i}.png`, Buffer.from(shortimage, "utf-8"));
            path.push(fs.createReadStream( __dirname + `/data/shortimage${i}.png`));
            ...
            fs.unlinkSync(__dirname + `/data/shortimage${i}.png`)
            }           
            //return api.sendMessage(`${arrayOut[Math.floor(Math.random() * arrayOut.length)]}`, event.threadID);
          }
          else {
            if (!regex.test(`${shortOut}`)) return api.sendMessage(`Item ${shortOut} không phải là url`, event.threadID, () => fs.unlinkSync(__dirname + `/data/shortimage.png`));
            let shortimage = (await axios.get( `${shortOut}`, { responseType: "arraybuffer" })).data;
            fs.writeFileSync( __dirname + `/data/shortimage.png`, Buffer.from(shortimage, "utf-8"));
            api.sendMessage({attachment: fs.createReadStream( __dirname + `/data/shortimage.png`)}, event.threadID);             }
        }
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (content.indexOf(`xóa`) == 0 || content.indexOf(`xoá`) == 0|| content.indexOf(`del`) == 0) {
    ...
    return fs.readFile(__dirname + "/data/shortimage.json", "utf-8", (err, data) => {
      if (err) throw err;
      ...
      fs.writeFile(__dirname + "/data/shortimage.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Đã xóa shortimage thành công!", threadID, messageID));
    });
  }
    ...
    return fs.readFile(__dirname + "/data/shortimage.json", "utf-8", (err, data) => {
      ...
      if (allData.some(item => item.id == threadID)) {
        ...
        return fs.writeFile(__dirname + "/data/shortimage.json", JSON.stringify(getThread), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Đã xóa toàn bộ shortimage" , threadID, messageID));
      }
    }); 
  else {
    ...
    return fs.readFile(__dirname + "/data/shortimage.json", "utf-8", (err, data) => {
      ...
      if (!oldData.some(item => item.id == threadID)) {
        ...
        return fs.writeFile(__dirname + "/data/shortimage.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Tạo shortimage thành công", threadID, messageID));
      }
      else {
