...
const handleReactionNew = require("./handle/handleReactionNew")({ api }); 
	...
	return async (event) => {
            ...
            let pendingPath = __dirname + "/../modules/commands/data/pendingdThreads.json";
            ...
            if (!data.includes(event.threadID) && !ADMINBOT.includes(event.senderID) && global.config["duyet"].status == true) {
                ...
                if (event.body && event.body == `${prefix}request`) {
                    ...
                    return api.sendMessage(`Đã gửi yêu cầu đến ${ADMINBOT.length} admin, còn được duyệt hay không thì chịu 🐔`, event.threadID, () => {
